...
TRAIL_ROOT_PATH=$(cd $(dirname $0) || exit; pwd)
echo "the root path of 'TRaIL-Odom': ${TRAIL_ROOT_PATH}"
...
cd ${TRAIL_ROOT_PATH}/thirdparty/ctraj
...
mkdir ${TRAIL_ROOT_PATH}/thirdparty/ctraj-build
# shellcheck disable=SC2164
cd "${TRAIL_ROOT_PATH}"/thirdparty/ctraj-build
...
echo "-----------------------"
make -j8
cmake --install . --prefix ${TRAIL_ROOT_PATH}/thirdparty/ctraj-install
